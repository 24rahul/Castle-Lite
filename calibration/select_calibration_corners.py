...
SAVE_PATH = "../config/calibration_corners.json"

class CalibrationCornerSelector:
    def __init__(self):
    ...
    def select_corners(self):
        print("\n=== Calibration Square Corner Selection ===")
        print("You will be prompted to move the camera to each calibration square corner.")
        ...
        for corner in CORNER_NAMES:
            print(f"\nMove to the {corner.replace('_', ' ').title()} of the calibration square and press 'C' to capture.")
            while True:
                ...
                cv2.imshow("Calibration Corner Selection", frame)
                ...
                if key == ord('q'):
                    ...
                    filename = os.path.join(img_dir, f"calib_{corner}_{timestamp}.jpg")
    ...
    def save_corners(self):
        ...
        print(f"\nCalibration square corner positions saved to: {SAVE_PATH}")
    ...
    def run(self):
        try:
            ...
            if self.select_corners():
                self.save_corners()
                print("\n=== Calibration square corner selection complete ===")
            else:
...
if __name__ == "__main__":
    selector = CalibrationCornerSelector()
    selector.run() 